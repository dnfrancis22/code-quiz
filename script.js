...
// I put it outside the loop because it kept printing the question five Time.
  ...
  for (var i = 0; i < gameContent[currentStage].answerChoices.length; i++) {
...
//   This is what happens when an answer is selected. The game will either end here or move to the next question.
answersContainer.addEventListener("click", function (event) {
  if (event.target.matches("button")) {
    ...
    if (currentStage === gameContent.length -1) {
      endGame();
    } else {
      currentStage++;
